...
### Fixed
- **kolaylistele.com frozen on English/"easylisting" defaults** (`templates/upgrade.html`, `index.html`, `bulk.html`, `listings.html`, `connect.html`, `photo_set.html`, `terms.html`, `privacy.html`, `form.html`): the `'strict-dynamic'` source added to `script-src` (commit `6c272da`) makes browsers ignore `'self'` and host allowlists, so parser-inserted `<script src=...>` tags without a nonce are blocked. The nonce-less Stripe (`js.stripe.com/v3/`) and `security.js` tags were dropped; on `/upgrade` that left `Stripe` undefined, so `const stripe = Stripe(...)` threw and aborted the inline block before `setLang()` ran — the page stayed on its English/"easylisting" static defaults (only the server-rendered `₺` currency was correct). Added `nonce="{{ csp_nonce }}"` to every external/static script tag (GTM/gtag were already nonced). Added a regression test (`test_external_scripts_carry_nonce_under_strict_dynamic`) asserting no nonce-less `<script src>` is rendered while `strict-dynamic` is in the CSP.
