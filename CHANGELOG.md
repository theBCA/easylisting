...
- **Wrong favicon on easylisting.app** (`static/favicon-e.svg`, `static/security.js`): both Railway services serve the same static `favicon.svg` ("K" tile), so easylisting.app showed kolaylistele's "K". Added an "E" tile favicon and a small hostname check in `security.js` (loaded early in every page head) that swaps the SVG icon to `favicon-e.svg` on any non-kolaylistele host. The swap now runs on `DOMContentLoaded` — the script loads in `<head>` before the favicon `<link>` is parsed, so the initial version found no element and never swapped.
